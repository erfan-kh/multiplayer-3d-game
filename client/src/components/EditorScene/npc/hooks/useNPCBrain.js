...
export function useNPCBrain({
  ...
  const cleanupSummonedCopies = useCallback(() => {
    setNpcs((prevNpcs) =>
      prevNpcs.filter(
        (otherNpc) => !(otherNpc.isSummoned && otherNpc.summonedByNpcId === npc.id)
      )
    );
  }, [npc.id, setNpcs]);
  ...
  useFrame((_, delta) => {
    ...
    if (girlRef?.current && startDialogue && npc.dialogue) {
      ...
      if (activeDialogueNpcId === npc.id) {
        ...
        flatLookTargetVec.set(playerPosVec.x, group.position.y, playerPosVec.z);
      ...
      if (distance < triggerRadius) {
        if (activeDialogueNpcId === null) {
          if (hasExitedRadiusRef.current && dialogueCooldownTimerRef.current <= 0) {
            hasExitedRadiusRef.current = false;
    ...
    if (npc.summonedToNpcId) {
      ...
      const distanceToCaller = currentPos.distanceTo(targetPosVec);

      if (distanceToCaller > SUMMON_STOP_DISTANCE) {
        isMoving = true;
      } else {
        flatLookTargetVec.set(targetPosVec.x, currentPos.y, targetPosVec.z);
        targetQuaternion.setFromRotationMatrix(
      ...
      if (detectedTargetPos && activeBehavior !== "ignore") {
        ...
        if (movementMode === "wander") {
          ...
          for (let checkedCount = 0; checkedCount < npc.waypoints.length; checkedCount += 1) {
            const candidateWaypoint = npc.waypoints[currentIndex];
            const candidatePosition = getWaypointPos(candidateWaypoint);

            if (candidatePosition && !isWaypointInsideObstacle(candidatePosition, currentIndex)) {
